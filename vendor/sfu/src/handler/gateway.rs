use crate::audio_callback::{is_audio_payload_type, AudioPacketInfo, AudioSender};
...
pub struct GatewayHandler {
    ...
    /// Optional sender for audio packets (for transcription, etc.)
...
impl GatewayHandler {
    ...
    /// Create a GatewayHandler with an audio callback for receiving decrypted audio packets.
    ///
    /// The callback will be invoked for every audio RTP packet that passes through the gateway.
    /// This is useful for transcription, recording, or other audio processing.
...
impl GatewayHandler {
    ...
    fn handle_rtp_message(
        ...
        // Send audio packets to the callback if configured
        if let Some(tx) = audio_tx {
            let pt = rtp_packet.header.payload_type;
            if is_audio_payload_type(pt) {
                let audio_info = AudioPacketInfo {
                ...
                // Non-blocking send - if receiver is slow, just skip
